...
io.out(2) := MuxLookup(Cat(io.sel(1).asUInt(), io.sel(0).asUInt()), false.B, Array(
    ...
    (0.U) -> (io.in(2)),
    (1.U) -> (io.in(3)),
    (2.U) -> (Mux(io.shift_type, 0.B, io.in(0))),
    (3.U) -> (Mux(io.shift_type, 0.B, io.in(1)))
    ))
io.out(3) := MuxLookup(Cat(io.sel(1).asUInt(), io.sel(0).asUInt()), false.B, Array(
    // B1
    (0.U) -> io.in(0),
    (1.U) -> (Mux(io.shift_type, 0.B, io.in(1))),
    (2.U) -> (Mux(io.shift_type, 0.B, io.in(2))),
    (3.U) -> (Mux(io.shift_type, 0.B, io.in(3)))
    ))
// End your code here   
}
